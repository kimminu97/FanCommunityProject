...
<div style="margin:auto;">
 <table>
	...
	<tr>
		<td>${vo.board_idx }</td> 	
 		<td><a href="detail?board_idx=${vo.board_idx}&pageNo=${page.pageNo}&action=1" class="title">${vo.board_sub }</a>
 	...
 	<tr><td colspan="4">Go!
 	<!-- a태그 url요청 get 메소드 -->
 	<a class="button" href="insert?pageNo=${page.pageNo }&action=1">글쓰기</a>
 ...
 <!-- 가장 첫페이지로 이동 -->
	<a class="pagenum" href="?pageNo=1&action=1">&lt;&lt;</a>   <!-- 요청url은 동일하고 파라미터만 변경됩니다. -->
	...
	<c:forEach var="i" begin="${page.startPage }" end="${page.endPage}">  <!-- 페이지목록의 범위  -->
		<a class="pagenum" href="?pageNo=${i}&action=1">${i}</a>     <!-- 현재페이지 i값으로 변경  -->
	</c:forEach>
	
	<a class="${page.endPage!=page.totalPage? 'pagenum':'none'}" href="?pageNo=${page.endPage+1 }&action=1">&gt;</a> 
	<a class="pagenum" href="?pageNo=${page.totalPage }&action=1">&gt;&gt;</a> <!-- 맨 마지막 페이지 -->
</div>
